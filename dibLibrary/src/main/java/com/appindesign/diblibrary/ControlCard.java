...
public class ControlCard 
{
	...
	public ControlCard( Context context )
	{
		...
		if ( isValid )
		{
			...
			{
				dib = new Punch( sCourseFields.get(i), Msg.INVALID_TIME );
				dibs.add( dib );
			}
		}
		else
	...
	public ControlCard( Cursor cursor )
	{
		...
		String sControls; String[] sControlsArray;		
		...
		if ( sControls !=null && sSplits != null )
		{
			...
			dibs = new ArrayList<Punch>();
			dib = new Punch( sControlsArray[0], Long.valueOf( sSplitsArray[0] )*Msg.TIME_DIVISOR );
			dibs.add( dib );
			for ( int i=1; i < sSplitsArray.length; i++ )
			{	
				dib = new Punch( sControlsArray[i], ( Long.valueOf( sSplitsArray[i] ) + lBaseTimestamp ) * Msg.TIME_DIVISOR );
				dibs.add( dib );
			}			
		}
